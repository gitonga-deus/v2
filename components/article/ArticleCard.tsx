...
export default function ArticleCard(article: ArticleCore) {
	...
	return (
		<Link
			...
			href={`articles/${slug}`}
		>
			<div className='flex items-center gap-2 pt-1 tracking-tight text-sm text-shade-2'>
			...
			<h1
				className='py-1 text-base  text-shade-1 font-semibold'
			>
				{title}
			</h1>
			<p className='text-[15px] text-shade-2'>{summary}</p>
		</Link>
	)
}